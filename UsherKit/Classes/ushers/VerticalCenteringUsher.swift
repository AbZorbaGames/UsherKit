...
public protocol VerticalCenteringUsher: Usher {}

public extension VerticalCenteringUsher {
    ...
    func positioning<Rect>(ofRects rects: [Rect], inBounds bounds: Rect) throws -> [Rect]
        where Rect: UsherRect {
            ...
            let centerY = (bounds.layoutHeight / 2)
            let positions = rects.map({ (rect: Rect) -> Rect in
                let y = centerY - (rect.layoutHeight / 2)
